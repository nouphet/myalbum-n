...
define("_ALBM_MYALBUM_DESC","建立相片簿以便用?可以 搜尋/發?/評論 分享各種相片");

//slideshow
define( "_ALBM_CFG_lINTERVAL" , "請指定秀圖的顯示切換方式。" ) ;
define( "_ALBM_CFG_DESClINTERVAL" , "秀圖時圖?切換的時間，預設是（5000=5秒）" ) ;
define( "_ALBM_CFG_TRANSITION" , "指定秀圖的切換方式。" ) ;
define( "_ALBM_CFG_DESCTRANSITION" , "可以指定秀圖的種類（transition）預設?是「23」隨機指定" ) ;
//slideshow end
//slideshow
...
define( "_ALBM_OPT_TRANSITION12" , "點?式隱現模式" ) ;
...
define("_ALBM_CFG_DESCPHOTOSPATH","路徑應該存在安裝到 XOOPS 系統的目?裡。<br />(路徑名稱應該以 '/'開始，而且不可以 '/' 結尾。)<br />而存放相片的目?權限屬性在 UNIX 系統設為 777 或 707。");
define("_ALBM_CFG_THUMBSPATH","縮圖存放的路徑");
define("_ALBM_CFG_DESCTHUMBSPATH","路徑應該存在安裝到 XOOPS 系統的目?裡。<br />(路徑名稱應該以 '/'開始，而且不可以 '/' 結尾。)<br />而存放相片的目?權限屬性在 UNIX 系統設為 777 或 707。.");
//define("_ALBM_CFG_USEIMAGICK","使用 ImageMagick 來處理圖?");
//define("_ALBM_CFG_DESCIMAGICK","選(否)表示使用 GD 。(將無法變更與旋轉相片)<br />所以請盡可能使用 ImageMagick。");
define("_ALBM_CFG_IMAGINGPIPE","相片處理套件");
define("_ALBM_CFG_DESCIMAGINGPIPE","幾乎 PHP 環境能使用 GD ，但是 GD 的功能低於?外二個套件。<br />如果可以，?最好使用 ImageMagick 或者 NetPBM。");
...
define("_ALBM_CFG_DESCMAKETHUMB","當?將 '否' 改為 '是'，?最好重新執行 '重建縮圖'。");
//define("_ALBM_CFG_THUMBWIDTH","縮圖寬度：");
//define("_ALBM_CFG_DESCTHUMBWIDTH","縮圖的高度會依據寬度的設定?自動完成.");
define("_ALBM_CFG_THUMBSIZE","縮圖的尺寸 (pixel)");
define("_ALBM_CFG_THUMBRULE","縮圖建立的計算規則");
define("_ALBM_CFG_WIDTH","相片最大寬度：");
define("_ALBM_CFG_DESCWIDTH","如果?使用 ImageMagick ，則代表寬度大小將會重新設定。<br />如果不是使用 ImageMagick，則代表這是寬度限制。");
define("_ALBM_CFG_HEIGHT","相片最大高度：");
define("_ALBM_CFG_DESCHEIGHT","如果?使用 ImageMagick ，則代表高度大小將會重新設定。<br />如果不是使用 ImageMagick ，則代表這是高度限制。");
define("_ALBM_CFG_FSIZE","最大?案大小：");
define("_ALBM_CFG_DESCFSIZE","限制上傳相片?案之大小 (byte)。<br />1048576 byte = 1 MB");
define("_ALBM_CFG_MIDDLEPIXEL","單張展示相片的最大尺寸");
define("_ALBM_CFG_DESCMIDDLEPIXEL","指定 (寬)x(高)<br />例如：480x480");
define("_ALBM_CFG_ADDPOSTS","當會員發?一張照片後,會員發表數所要添加數為：");
...
define("_ALBM_CFG_ALLOWEDEXTS","允許上傳的副?名");
define("_ALBM_CFG_DESCALLOWEDEXTS","輸入副?名並以 \'|\' 分隔。(例如：\'jpg|jpeg|gif|png\')<br />字元必須全部是小寫，不要插入句號(.)或空白鍵。");
define("_ALBM_CFG_ALLOWEDMIME","允許上傳的 MIME 類型");
define("_ALBM_CFG_DESCALLOWEDMIME","輸入 MIME 類型並以 \'|\' 分隔。(例如：\'image/gif|image/jpeg|image/png\')<br />在這裡空白時，代表?想 MIME 類型檢?。");
define("_ALBM_CFG_USESITEIMG","使用 [siteimg] 在圖案管理員的整合");
define("_ALBM_CFG_DESCUSESITEIMG","圖案管理員的整合中以 [siteimg] 取代 [img]。<br />如沒有?必須 HACK module.textsanitizer.php ?用 [siteimg] 標籤。<br />XOOPS標準預設並無此項。");
...
define("_ALBUM_OPT_CALCWHINSIDEBOX","無論長寬，以較大?一方自動調整");
...
define("_ALBM_MYALBUM_ADMENU3","檢?組態與環境設定");
